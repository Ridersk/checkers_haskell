...
listBoundaries ((ix, jx) : es) = foldr updateBoundaries ((ix, jx), (ix, jx)) es
  where
    updateBoundaries (x, y) ((minX, minY), (maxX, maxY)) = ((minX', minY'), (maxX', maxY'))
      where
...
gameBoardMovePiece :: Ix index => (index, index) -> (index, index) -> GameBoard index e -> GameBoard index e -> IO () -- GameBoard index e)
gameBoardMovePiece posO posD gbPiecesOrig gbPiecesDest = do
  piece <- gameBoardGetPiece posO gbPiecesOrig
  maybe
    (return ())
    ( \piece' -> do
        gameBoardRemovePiece posO gbPiecesOrig
        gameBoardSetPiece posD piece' gbPiecesDest
    )
    piece
